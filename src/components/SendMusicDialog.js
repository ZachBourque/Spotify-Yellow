...
class SearchUsers extends Component {
    ...
    componentDidMount() {
        axios.get('/users').then(res => {
            this.setState({ users: res.data.users, topic: this.props.element })
        })
    }
    ...
    handleClick = () => {
        const {url, image: pic} = this.state.topic
        const {token, expires, rtoken} = this.props.auth
        this.props.sendMusic(url, pic, this.state.selectedUser.id, token, expires, rtoken ).then(res => {
            console.log("SUCCESS");
            this.props.onClose();
        })   
    }
            ...
            <Dialog open={this.props.open} onClose={this.props.onClose} maxWidth='sm' fullWidth>
                ...
                <DialogContent>
                    <TextField
                        ...
                        autoComplete="off"
                    />
                    {this.state.searchText && <Card>{this.state.filteredUsers.map((user) => {
